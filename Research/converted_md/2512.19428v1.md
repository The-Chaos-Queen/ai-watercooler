...
pairsviaadenseorapproximateattentionmechanism. Inthispaperwequestionthatassumption.
...
Asacontrastingdesign,weproposeanattention-free sequencemodelbuiltaroundGrassmann
flows. InsteadofforminganL×Lattentionmatrix,we(i)reducetokenstatestoalow-dimensional
space, (ii) interpret local token pairs as two-dimensional subspaces on a Grassmann manifold
Gr(2,r), (iii) embed these subspaces via Plu¨cker coordinates into a finite-dimensional projective
...
slightlyoutperforms aTransformerhead(bestvalidationaccuracy0.8550vs.0.8545;testaccuracy
...
Conceptually, self-attention performs a form of tensor lifting. From a hidden vector h ∈ Rd at
t
positiont,wemovetoaspacethatencodespairwiserelationsbetweentokens—anL×Lcompatibility
...
• Itishigh-dimensional: theeffectivestateofthemodelinvolvesnotonlythetokenembeddings
...
2. Attention implements a specific kind of tensor lifting: it takes the vector h and, via dot
t
...
| --------- | --- | --------- | --- | --- | ------------ | --- | ------------------- | --- | --- | --- |
• We first apply a learned linear map W to reduce h ∈ Rd to z ∈ Rr, with r ≪ d.
|     |     |     |     |     |     |     | red |     | t   | t   |
| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |
•
For local windows (e.g., pairs (t,t+∆)), we consider the subspace spanned by {z ,z } in
t t+∆
...
• These Plu¨cker coordinates become geometric features that we project back into Rd and fuse
...
|     | (Grassmannian |     |     | + Plu¨cker | relations)[11, |     | 12, | 13, 14]. |     |     |
| --- | ------------- | --- | --- | ---------- | -------------- | --- | --- | -------- | --- | --- |
...
on Gr(2,r) via Plu¨cker coordinates, and (iii) fuses these geometric features back into the
...
Grassmannmixingandshowthat, forfixedrankandwindowsizes, itscaleslinearlyinsequence
...
|      |          |       |     |              |       |           | h        | V            |       |     |     |
|      | (h)      | (h)   | (h) |              |       |           |          |              |       |     |     |
| with | W ,W     | ,W    |     | ∈ Rd×d       | h and | typically | d        | = d/H        | .     |     |     |
|      | Q        | K     | V   |              |       |           | h        | heads        |       |     |     |
...
The outputs O are concatenated and linearly projected back to Rd.
h
This process can be interpreted as follows:
• The linear maps W (h) ,W (h) embed the sequence into a representation where dot products
Q K
encode compatibilities between positions.
• The matrix Q K⊤ is a lift from H into a space of pairwise interactions: each token is
h h
represented not just by a vector, but by its similarities to all other tokens.
• The softmax and multiplication by V implement a specific geometric operation on this lifted
h
...
TheGrassmannmanifoldGr(k,r)isthesetofallk-dimensionallinearsubspacesofRr. Itisasmooth
...
There are several standard ways to represent points on a Grassmannian. We use the Plu¨cker
embedding, which maps each k-dimensional subspace to a point in projective space:
• Given a basis (u ,...,u ) of a k-dimensional subspace U ⊂ Rr, we form the exterior product
1 k
u ∧···∧u in the k-th exterior power ΛkRr.
1 k
• In coordinates, u 1 ∧···∧u k can be represented as a vector in R( k r) whose entries are the k×k
minors of the matrix [u ... u ].
1 k
...
.
The image of Gr(2,r) under this embedding is not all of
R(r
2
)
; it is an algebraic variety defined
by the quadratic Plu¨cker relations. Nonetheless, for our purposes we can simply regard p as a
normalized feature vector encoding the subspace spanned by u and v. Different bases spanning the
same subspace yield proportional Plu¨cker vectors, reflecting the projective nature of the embedding.
...
Local linear structure. Onasmoothmanifold, localgeometrycanbecapturedbytangentspaces
...
sits inside a projective space via the Plu¨cker embedding. This means we can encode geometric
...
| ---------- | ------ | ------ | --- | ----------------- | --- | ------ | --- | --- | --- |
R(r )
2. constructs local pairs and encodes them as Plu¨cker vectors in 2 , and
3. mixesthesegeometricfeaturesbackintotheoriginalhiddenstatesviagatingandafeed-forward
network.
...
Given a sequence of tokens (x ,...,x ), we embed them into Rd using a learned embedding matrix
|      |         |            |     | 1          | L         |          |              |     |     |
| ---- | ------- | ---------- | --- | ---------- | --------- | -------- | ------------ | --- | --- |
| RV×d |         |            |     |            |           | RLmax×d: |              |     |     |
| E ∈  | and add | positional |     | embeddings | P         | ∈        |              |     |     |
|      |         |            |     | h (0)      | = E(x )+P | ,        | t = 1,...,L. |     |     |
|      |         |            |     | t          | t         | t        |              |     |     |
(0) (0)
Throughout our experiments we use d = 256. The resulting sequence H(0) = (h ,...,h ) is fed
1 L
| through    | N stacked | Causal | Grassmann |        | Transformer |     | layers. |     |       |
| ---------- | --------- | ------ | --------- | ------ | ----------- | --- | ------- | --- | ----- |
| 3.2 Causal | Grassmann |        |           | mixing | layer       |     |         |     |       |
|            |           |        |           | RL×d   |             |     |         | H˜  | RL×d. |
Each layer takes as input H ∈ and outputs an updated sequence ∈ The core
...
| --- | --- | --- | --- | --- | ---- | --- | --- | ----- | ------- |
|     |     |     | t   | red | t    | red | red |       | red     |
Typically r ≪ d; in our experiments we use r = 32. This yields Z = (z ,...,z ) ∈ RL×r.
1 L
2. Multi-scale local pairing. We define a set of window sizes (offsets) W = {∆ ,...,∆ }, e.g.,
1 m
|     |     |     |     |     | W = | {1,2,4,8,12,16} |     |     |     |
| --- | --- | --- | --- | --- | --- | --------------- | --- | --- | --- |
or a multi-layer schedule such as (1,1,2,2,4,4,8,8,12,12,16,16) for deeper models. For each
position t and offset ∆ ∈ W such that t+∆ ≤ L, we form a pair (z t ,z t+∆ ).
...
3. Grassmann / Plu¨cker encoding. For each pair (z ,z ), we consider the 2D subspace
|     |     |     |     |     |     |     |     | t   | t+∆        |
| --- | --- | --- | --- | --- | --- | --- | --- | --- | ---------- |
|     |     |     |     |     |     |     |     |     | ( ∆) R(r ) |
spanned by these vectors in Rr. We form the Plu¨cker vector p ∈ whose entries are:
|     |     |     |     |     |     |     |     |     | t 2 |
| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |
(∆)
|     |     |     | p   | (t) = | z t,i z t+∆,j | −z t,j | z t+∆,i | , 1 ≤ | i < j ≤ r. |
| --- | --- | --- | --- | ----- | ------------- | ------ | ------- | ----- | ---------- |
ij
...
|           |           |         |          |         |              | |W |    |          |            |         |
|           |           |         |          |         |              | t       | ∆∈Wt     |            |         |
where W t = {∆ ∈ W : t+∆ ≤ L} is the set of valid offsets at position t.
The vector g ∈ Rd captures multi-scale local Grassmann geometry around position t.
t
...
|           |                |               | hˆ  | = LayerNorm(h˜mix); |          |         | hˆ       | = Dropout(hˆ | ).     |
|           |                |               |     | t                   |          | t       | t        |              | t      |
7
...
| --- | --- | --- | --- | --- | --- | --------- | --------- | --- | --- |
|     |     |     |     |     |     | t 2       | 1 t 1     | 2   |     |
with W ∈ Rd ff ×d, W ∈ Rd×d ff, d = 4d, and a nonlinearity σ (we use GELU). Another residual
|            |            | 1   |        | 2                 | ff  |                   |            |              |     |
| ---------- | ---------- | --- | ------ | ----------------- | --- | ----------------- | ---------- | ------------ | --- |
...
| --- | --- | --- | --- | --- | --- | --------- | --- | --- | --- |
head
where d is the per-head dimension. The first term arises from computing Q,K,V, and the
head
...
|     | • Linear  |           | reduction: | HW⊤    | costs  | O(Ldr).  |            |     |     |
red
(∆)
• Plu¨cker computation: for each position and offset, forming p costs O(r2). With m = |W|
t
O(Lmr2).
|     | offsets,   |     | this contributes |              |     |               |                 |                  |           |
| --- | ---------- | --- | ---------------- | ------------ | --- | ------------- | --------------- | ---------------- | --------- |
|     | •          |     |                  |              |     | (∆)           | (cid:0)r(cid:1) |                  | O(Lmdr2). |
|     | Projection |     | to               | model space: | W   | plu¨ pˆ costs | O(d )           | per pair, giving |           |
|     |            |     |                  |              |     | t             | 2               |                  |           |
...
note that r2 is modest, then the Plu¨cker and projection costs can be absorbed into the O(Ld2) term.
Crucially, there is no L2 term: the complexity is linear in L for fixed r and m:
|     |     |     |        |            |     | O(Ld2) |                     | O(L2d | +Ld2). |
| --- | --- | --- | ------ | ---------- | --- | ------ | ------------------- | ----- | ------ |
|     |     |     | Causal | Grassmann: |     |        | vs. Self-attention: |       |        |
head
In practice, our current implementation is slower per step than highly optimized GPU attention
kernels for moderate L, due to overheads in computing Plu¨cker coordinates and handling reshapes.
...
d = 256, feed-forward dimension d ff = 1024, and multi-head self-attention with 4 heads.
• GrassmannLM: the same backbone (embeddings, number of layers, d, d ), but with each
ff
...
| ----------------- | --------- | ---------- | --------------------- | --- | ------ |
Backbone. Forfaircomparison, wefixaDistilBERT-base-uncasedbackboneasafeatureextractor.
...
| -------- | ----------- | ------ | --------------- | --------------- | --- |
•
Grassmann–Plu¨ckerhead: ourproposedGrassmann-basedhead(GrassmannPluckerNLIModel),
...
| --- | ------------------------ | --- | ----------- | --- | --- |
The Grassmann head uses the following hyperparameters: reduced dimension d = 64, window
proj
size 8 with stride 8 over the token sequence, d = 256, 2 mixing layers, 4 mixing heads (for
model
grouping pairs), d = 512, and dropout 0.1. Both heads have comparable parameter counts, and
ff
...
fortheTransformerLMundermatchedtrainingconditions. Withaslightlydifferentwindowschedule
...
the Plu¨cker projection and gating layers, but the difference is on the order of ∼ 3–5%.
...
Grassmann–Plu¨cker head 0.8550 0.8538
Table 3: SNLI classification accuracy with DistilBERT backbone. The Grassmann–Plu¨cker head
...
mechanisms.
• Our Plu¨cker computation involves explicit element-wise operations and reshaping that do not
...
attention. Withrelativelysmallmodelsandmodestcontextlengths, theproposedCausalGrassmann
architecture:
...
Self-attentionletseachtokenseeeveryothertokenviaalearnedL×Lweightmatrix. Grassmann
mixing, by contrast, constructs a sequence of local subspace updates: information flows by rotating
andbendinglow-ranksubspacesovermulti-scalewindows. Bothmechanismsaccumulatehigher-order
...
• The reduced states z ∈ Rr capture local directions in a lower-dimensional space.
t
• Pairs (z ,z ) define points on Gr(2,r); these points are encoded by Plu¨cker vectors with
t t+∆
fixed dimension
(cid:0)r(cid:1)
.
...
This suggests a more hopeful interpretability story. After training, one can treat the Plu¨cker
...
sequence.
• Sequence-level statistics of Plu¨cker coordinates, such as principal directions or curvature-like
quantities.
•
...
temporaldynamicswithGrassmannconstraintsonhiddenrepresentationsareaninterestingdirection
for future work.
...
Theiruseasaprimarymixingmechanisminsequencemodelshasbeenlessexplored. Ourcontribution
is to integrate a Grassmann–Plu¨cker pipeline directly into a Transformer-like block, turning subspace
...
• Interpretability studies. Systematically investigate correlations between Plu¨cker coordi-
...
• Scaling and engineering. ImplementfusedGrassmannkernelsandoptimizedGPUoperators
...
Insummary, ourresultssuggestthatwhatwefundamentallyneedforstrongsequencemodelingis
...
Transformer,”inProceedings of the 35th International Conference on Machine Learning (ICML),
| pp. 4055–4064,      | 2018.               |                 |
| ------------------- | ------------------- | --------------- |
