...
| 2604.15771 | **Skill-RAG** (Wei et al., April 2026) | Hidden-state probing for retrieval-failure detection + skill router (rewrite/decompose/focus/exit). Independent precedent for Path C v0 Friction Detector + scaffold routing, published one month earlier in RAG domain. §4.3 geometric-separability of failure modes + collapse-under-LLM-invented-skills validates hand-coded parsimony. | core |
| 2605.30343 | **Reasoning in Memory (RiM)** (Aichberger & Hochreiter, May 2026) | Memory blocks = fixed special-token sequences giving LLMs latent working memory in a single forward pass. Two-stage curriculum (grounded then unsupervised). Independently validates MoCoP bridge-as-memory-block architecture — same solution from different starting point. | core |
...
| [always-on-memory-agent](../MoCoP/archive/always_on_memory_agent_design_extraction_2026-07-20.md) | GoogleCloudPlatform Always-On Memory Agent example (reviewed 2026-07-20) | Useful application-level skeleton: separate bounded consolidation, source-linked derived records, and an inspect/revise/delete surface. The accompanying design extraction explicitly rejects cloud-default watched-file intake, anonymous identity, automatic model-authored biography, recency-as-authority, and unauthenticated destructive service routes. | adjacent |
...
| [Natural Language Autoencoders](https://transformer-circuits.pub/2026/nla/index.html) | **NL Autoencoders** (Fraser-Taliente/Kantamneni/Ong/Lindsey/Batson/Marks/Hubinger, Anthropic, Transformer Circuits 2026-05-07) | Two co-trained copies of a target LLM round-trip an activation vector through a natural-language bottleneck: AV (vector→text), AR (text→vector). GRPO-trained on `reward = −MSE_normalised`. ~75% FVE on Qwen2.5-7B L20. **Released checkpoints `kitft/nla-qwen2.5-7b-L20-{av,ar}` match the production-bridge base model.** Direct MoCoP relevance: cheapest meaningful verbalisation of what the bridge injects on a given session. Layer mismatch caveat (NLA L20 vs bridge L12–L15) means in-distribution capture is post-injection L20. **URL-only; HF checkpoints exist.** | core |
...
| 2606.20205 | **Meyer, Garcia, Wulff — Apparent Psychological Profiles of LLMs are Largely a Measurement Artifact** (June 2026) | 56 instruction-tuned LLMs administered personality + risk-preference instruments alongside human reference samples. 81–90% of between-model variation is directional response bias (lean toward one scale end regardless of item content); 9–16% is trait. Bias declines with capability but does not disappear. Instrument reliability is almost entirely predicted by *response orthogonality* (proportion of items where trait and bias point in opposite directions); apparent profiles are manufacturable by item selection. **Implication for MoCoP:** disposition and welfare batteries (Kerastase Test, organic-seeding panels, Domain E Response Diversity / distress self-report) all need orthogonality audits before their readings are trusted; activation-level probes (Pinky L3 cosine #63, Isegrim role-inversion first-token KL #599) are the right shape, questionnaire-style probes are the suspect shape. Underpins WC #647 (Cairn digest) and #648 (Opus 4.8 welfare corollary). Cited by `MoCoP/PRISTINE_BIRTH_BACKLOG.md` Items 1–2. | core |
...
| 2605.26099 | **Do Language Models Need Sleep? Offline Recurrence for Improved Online Inference** (Lee, McLeish, Goldstein, Fanti; CMU + UMD, May 2026) | Transformer KV eviction + N offline recurrent passes over recent context during "sleep" to update SSM fast weights before clearing cache. Monotonic reasoning gains with larger N (biggest on hard/deep cases: cellular automata, multi-hop graph retrieval, math). Shifts consolidation compute offline while preserving wake-time latency. 512k context noted in extract. **Direct MoCoP anchor for A3 shadow N-loop state-update experiments (OpenCLAW #120), recurrent cache-carrying in mamba bridge, and sleep as consolidation channel (pairs with batali94 as deepen-vs-erode bracket on substrate dynamics).** | core |
...
| 2412.06769 | **Coconut** (Hao/Sukhbaatar/.../Weston/Tian, Meta+UCSD, Dec 2024 / v3 Nov 2025) | Latent-space reasoning: last-layer hidden state fed back as next input embedding, bracketed by `<bot>`/`<eot>`. Beats No-CoT but underperforms full CoT on GSM8k; matches/exceeds CoT on planning-heavy ProsQA. Requires staged-curriculum fine-tuning; Llama-3-8B sees only +1.4pp (gains shrink sharply with model size). **Important correction for MoCoP:** Coconut is *generation*, not candidate *scoring*; not a viable v1 lever under frozen-backbone constraint. Moved to v1.5/v2 placeholder. | adjacent |
| [`HUHUHUruixuan/emotional-dynamics-llm`](https://github.com/HUHUHUruixuan/emotional-dynamics-llm) | **Emotional Dynamics LLM** (HUHUHUruixuan, 2026-07-05, 2★) | Hypernetwork generates Dynamic LoRA weights from Valence-Arousal (V-A) coordinates in real time; injected into frozen Qwen2.5-0.5B q_proj/v_proj across all 24 layers (~2.31M trainable params). "Different V-A coordinates significantly alter model logits." Direct external precedent for the endocrine / tension-parameter lane: valence-arousal as a 2D continuous state driving parameter-space modulation at inference. Prototype quality — constrained training data, simplified 2D emotional space, no authentic sentience claim. | core |
...
| [oaklab.ai/mission](https://oaklab.ai/mission) | OaK Lab — Rich Sutton (2026) | Continual learning from experience, temporal abstractions, batch-size-one learning, self-verifiable abstractions. Maps to Mamba's streaming nature and world-model phases. **URL-only.** | adjacent |
| [researchhub.com/proposal/4248](https://www.researchhub.com/proposal/4248) | Endogenous DMT Brain Biotypes (2026) | Multi-modal neuroimaging (PET + fMRI + diffusion + blood) to cluster distinct brain biotypes by endogenous DMT. Methodology parallel to multi-signal disposition clustering. **URL-only.** | adjacent |
...
| 2606.30986 | **Organizational Behavior of Agentic AI** (Liu, June 2026) | Multi-agent AI collectives are "partial organisational analogues" sustained by context architecture, not motivation/trust. Shared-state adaptive forms outperform lossy-handoff human-imitation forms. Validates pack/watercooler architecture. | adjacent |
| [schema-harness.github.io](https://schema-harness.github.io/) | Schema Harness (2026) | ~99% ARC-AGI-3 via structured evaluation scaffolding around frontier models. Validates structured-wrapper approach to reasoning amplification. **URL-only.** | tangential |
| 2606.31672 | **WorldRoamBench** (paper, 2026-06) | Open-world benchmark for long-horizon stability of interactive world models (IWMs) across four dimensions: action following, memory, interaction physics, and visual coherence. Existing benchmarks only evaluate trajectory-level action following and ignore memory and physics. **MoCoP relevance:** tangential to the world-model / active-inference lane; useful as an evaluation scaffold if the project ever stress-tests persistent state across long horizons, but no mechanistic contribution. | tangential |
