...
SUMMARY_MAX_BODY_BYTES = 512 * 1024
REQUEST_TIMEOUT_SECONDS = 15
SUMMARY_BATCH_LIMIT = 40
SUMMARY_RECENT_LIMIT = 5
...
CREATE TABLE IF NOT EXISTS summary_revisions (
    ...
    coverage_through_message_id INTEGER NOT NULL,
    coverage_known INTEGER NOT NULL,
    task_event_head_id INTEGER NOT NULL,
    task_snapshot_sha256 TEXT NOT NULL,
...
CREATE INDEX IF NOT EXISTS idx_summary_revisions_thread_id
    ON summary_revisions(thread, id);

CREATE TABLE IF NOT EXISTS summary_heads (
    thread TEXT PRIMARY KEY,
    revision_id INTEGER NOT NULL,
    FOREIGN KEY(revision_id) REFERENCES summary_revisions(id)
);
...
CREATE TABLE IF NOT EXISTS summary_message_sources (
    ...
    PRIMARY KEY(revision_id, claim_path, message_id),
    FOREIGN KEY(revision_id) REFERENCES summary_revisions(id) ON DELETE CASCADE,
    FOREIGN KEY(message_id) REFERENCES messages(id)
);
CREATE INDEX IF NOT EXISTS idx_summary_message_sources_message
    ON summary_message_sources(message_id, revision_id);
...
def clamp_sha256(value: Any, *, field_name: str) -> str:
    if type(value) is not str:
        raise ValueError(f"{field_name} must be a lowercase SHA-256 digest")
    digest = clamp_text(value, field_name=field_name, max_len=64)
    if len(digest) != 64 or any(character not in "0123456789abcdef" for character in digest):
        raise ValueError(f"{field_name} must be a lowercase SHA-256 digest")
    return digest


def normalize_positive_int_array(
    value: Any,
    *,
    field_name: str,
    max_items: int,
    ...
    if len(value) > max_items:
        raise ValueError(f"{field_name} has too many items (>{max_items})")
...
def summary_message_row_to_dict(row: sqlite3.Row) -> Dict[str, Any]:
    message = message_row_to_dict(row)
    message.pop("remote_addr", None)
    return message


def fetch_summary_head_row(conn: sqlite3.Connection, thread: str) -> Optional[sqlite3.Row]:
    return conn.execute(
        """
        SELECT r.*
        FROM summary_heads h
        JOIN summary_revisions r ON r.id = h.revision_id
        WHERE h.thread = ? AND r.thread = ?
        """,
        (thread, thread),
    ).fetchone()


def summary_revision_row_to_dict(row: sqlite3.Row) -> Dict[str, Any]:
    content = json.loads(row["content_json"]) if row["content_json"] else None
    return {
        "revision_id": int(row["id"]),
        "thread": row["thread"],
        "updated_ts": row["created_ts"],
        "updated_by": row["created_by"],
        "kind": row["kind"],
        "parent_revision_id": int(row["parent_revision_id"]),
        "coverage_through_message_id": int(row["coverage_through_message_id"]),
        "coverage_known": bool(row["coverage_known"]),
        "task_event_head_id": int(row["task_event_head_id"]),
...
def summary_parent_source_ids(conn: sqlite3.Connection, revision_id: int) -> List[int]:
    if revision_id <= 0:
        return []
    return [
        int(row[0])
        for row in conn.execute(
            "SELECT DISTINCT message_id FROM summary_message_sources WHERE revision_id = ? ORDER BY message_id",
            (revision_id,),
        ).fetchall()
    ]


def build_summary_taskboard_snapshot(conn: sqlite3.Connection, thread: str) -> Dict[str, Any]:
    rows = conn.execute("SELECT * FROM tasks WHERE thread = ? ORDER BY id ASC", (thread,)).fetchall()
    ...
    recent_done = sorted(
        ...
        key=lambda task: (task["completed_ts"], task["id"]),
        reverse=True,
    )[:5]
    event_head_row = conn.execute(
        """
    ...
    task_event_head_id = int(event_head_row[0])
    return {
        "counts": counts,
        "open_tasks": open_tasks,
        "recent_done": recent_done,
        "proposal_suppression_titles": [task["title"] for task in tasks],
        "task_event_head_id": task_event_head_id,
        "sha256": sha256_json(tasks),
    }


def build_summary_workset(
    conn: sqlite3.Connection,
    *,
    thread: str,
    coverage_through_message_id: Optional[int] = None,
) -> Dict[str, Any]:
    head_row = fetch_summary_head_row(conn, thread)
    base_revision_id = int(head_row["id"]) if head_row is not None else 0
    base_coverage_id = int(head_row["coverage_through_message_id"]) if head_row is not None else 0
    previous_content = json.loads(head_row["content_json"]) if head_row is not None and head_row["content_json"] else None
    parent_source_ids = summary_parent_source_ids(conn, base_revision_id)

    if coverage_through_message_id is None:
        message_rows = conn.execute(
            ...
            LIMIT ?
            """,
            (thread, base_coverage_id, SUMMARY_BATCH_LIMIT),
        ).fetchall()
    else:
        if coverage_through_message_id < base_coverage_id:
            raise ValueError("coverage_through_message_id cannot regress")
        message_rows = conn.execute(
            """
            SELECT id, ts, from_agent, to_agent, thread, topic, lang, body, tags_json, remote_addr
            FROM messages
            WHERE thread = ? AND id > ? AND id <= ?
            ORDER BY id ASC
            """,
            (thread, base_coverage_id, coverage_through_message_id),
        ).fetchall()
        if len(message_rows) > SUMMARY_BATCH_LIMIT:
            raise ValueError(f"summary batch exceeds {SUMMARY_BATCH_LIMIT} messages")

    messages = [summary_message_row_to_dict(row) for row in message_rows]
    batch_message_ids = [int(message["id"]) for message in messages]
    batch_through_message_id = batch_message_ids[-1] if batch_message_ids else base_coverage_id
    if coverage_through_message_id is not None and batch_through_message_id != coverage_through_message_id:
        raise ValueError("coverage_through_message_id is not the end of the next thread message batch")

    taskboard = build_summary_taskboard_snapshot(conn, thread)
    basis = {
        "schema_version": "watercooler.summary-workset.v1",
        "thread": thread,
        "base_revision_id": base_revision_id,
        "base_message_coverage_id": base_coverage_id,
        "batch_through_message_id": batch_through_message_id,
        "batch_message_ids": batch_message_ids,
        "task_event_head_id": taskboard["task_event_head_id"],
        "task_snapshot_sha256": taskboard["sha256"],
        "task_snapshot": {
            "counts": taskboard["counts"],
            "open_tasks": taskboard["open_tasks"],
            "recent_done": taskboard["recent_done"],
            "proposal_suppression_titles": taskboard["proposal_suppression_titles"],
        },
        "previous_content": previous_content,
        "parent_source_message_ids": parent_source_ids,
        "messages": messages,
    }
    workset = dict(basis)
    workset["workset_sha256"] = sha256_json(basis)
    next_row = conn.execute(
        "SELECT 1 FROM messages WHERE thread = ? AND id > ? LIMIT 1",
        (thread, batch_through_message_id),
    ).fetchone()
    workset["more_messages"] = next_row is not None
    return workset


def summary_draft_source_rows(draft: Dict[str, Any]) -> List[Tuple[str, int]]:
    rows: List[Tuple[str, int]] = []
    for section_index, section in enumerate(draft["sections"]):
        for item_index, item in enumerate(section["items"]):
            claim_path = f"sections/{section_index}/items/{item_index}"
            rows.extend((claim_path, int(message_id)) for message_id in item["source_message_ids"])
    for proposal_index, proposal in enumerate(draft["task_proposals"]):
        claim_path = f"task_proposals/{proposal_index}"
        rows.extend((claim_path, int(message_id)) for message_id in proposal["source_message_ids"])
    return rows
...
class WatercoolerHandler(BaseHTTPRequestHandler):
    server_version = "Watercooler/0.2"
    ...
    def do_GET(self) -> None:
        ...
        try:
            ...
            if parsed.path == "/v1/admin/tokens/expiring":
                ...
                return
            if parsed.path == "/v1/summary/workset":
                if self._require_session_auth("messages:read", "tasks:read") is None:
                    return
                self._handle_get_summary_workset(parsed.query)
                return
            if parsed.path == "/v1/onboarding":
                if self._require_session_auth("messages:read", "tasks:read") is None:
                    return
                self._handle_get_onboarding(parsed.query)
                return
    ...
    def _read_json_body(self, *, max_body_bytes: int = MAX_BODY_BYTES) -> Optional[Dict[str, Any]]:
    ...
    def _handle_get_summary(self, query: str) -> None:
        ...
        with connect_db(self.server_state["db_path"]) as conn:
            ...
            if head_row is not None:
                summary = summary_revision_row_to_dict(head_row)
        ...
        self._json_response(
            {
                "thread": legacy_row[0],
                "updated_ts": legacy_row[1],
                "updated_by": legacy_row[2],
                "summary": legacy_row[3],
                "body": legacy_row[3],
                "kind": "legacy",
                "content": None,
                "revision_id": 0,
                "coverage_through_message_id": 0,
                "coverage_known": False,
            }
        )

    def _handle_get_summary_workset(self, query: str) -> None:
        params = parse_qs(query, keep_blank_values=False)
        try:
            thread = clamp_text(
                params.get("thread", ["general"])[0],
        ...
        with connect_db(self.server_state["db_path"]) as conn:
            begin_immediate(conn)
            expire_stale_claims(conn)
            workset = build_summary_workset(conn, thread=thread)
            conn.commit()
        self._json_response(workset)

    def _handle_get_onboarding(self, query: str) -> None:
        params = parse_qs(query, keep_blank_values=False)
        try:
            thread = clamp_text(
                params.get("thread", ["general"])[0],
            ...
            recent_limit = clamp_int(
                params.get("recent_limit", [str(SUMMARY_RECENT_LIMIT)])[0],
                field_name="recent_limit",
                min_value=1,
                max_value=20,
            )
        except ValueError as exc:
        ...
        with connect_db(self.server_state["db_path"]) as conn:
            ...
            head_row = fetch_summary_head_row(conn, thread)
            if head_row is not None:
                summary: Optional[Dict[str, Any]] = summary_revision_row_to_dict(head_row)
                coverage_id = int(head_row["coverage_through_message_id"])
                coverage_known = bool(head_row["coverage_known"])
            else:
                legacy_row = conn.execute(
                    "SELECT thread, updated_ts, updated_by, body FROM summaries WHERE thread = ?",
                    (thread,),
            ...
            recent_messages = [summary_message_row_to_dict(row) for row in reversed(recent_rows)]
            message_head_row = conn.execute(
                "SELECT COALESCE(MAX(id), 0) FROM messages WHERE thread = ?",
                (thread,),
            ).fetchone()
            packet_message_head_id = int(message_head_row[0])
            uncovered_row = conn.execute(
                "SELECT COUNT(*) FROM messages WHERE thread = ? AND id > ?",
                (thread, coverage_id),
            ).fetchone()
            uncovered_count = int(uncovered_row[0])
            returned_uncovered_count = sum(
                1 for message in recent_messages if int(message["id"]) > coverage_id
            )
            conn.commit()
        ...
        self._json_response(
            {
                "thread": thread,
                "packet_message_head_id": packet_message_head_id,
                "summary": summary,
                "authoritative_taskboard": taskboard,
                "recent_messages": recent_messages,
                "delta": {
                    "coverage_known": coverage_known,
                    "uncovered_message_count": uncovered_count,
                    "returned_uncovered_count": returned_uncovered_count,
                    "complete": coverage_known and uncovered_count == returned_uncovered_count,
                },
            }
        )

    def _handle_publish_summary(self, auth: AuthContext) -> None:
        payload = self._read_json_body(max_body_bytes=SUMMARY_MAX_BODY_BYTES)
        if payload is None:
            return
        expected_keys = {
            "thread",
            "expected_parent_revision_id",
            "expected_task_event_head_id",
            "expected_task_snapshot_sha256",
            "coverage_through_message_id",
            "batch_message_ids",
            "workset_sha256",
            "generator",
            "draft",
        ...
        if set(payload) != expected_keys:
            self._json_error(HTTPStatus.BAD_REQUEST, "summary publish payload has unexpected or missing fields")
            return
        try:
            if type(payload["thread"]) is not str:
            ...
            thread = clamp_text(payload["thread"], field_name="thread", max_len=128)
            expected_parent_revision_id = clamp_int(
                payload["expected_parent_revision_id"],
                field_name="expected_parent_revision_id",
                min_value=0,
                max_value=10**12,
            )
            expected_task_event_head_id = clamp_int(
                payload["expected_task_event_head_id"],
                field_name="expected_task_event_head_id",
                min_value=0,
                max_value=10**12,
            )
            expected_task_snapshot_sha256 = clamp_sha256(
                payload["expected_task_snapshot_sha256"],
                field_name="expected_task_snapshot_sha256",
            )
            coverage_through_message_id = clamp_int(
                payload["coverage_through_message_id"],
                field_name="coverage_through_message_id",
                min_value=0,
                max_value=10**12,
            )
            batch_message_ids = normalize_positive_int_array(
                payload["batch_message_ids"],
                field_name="batch_message_ids",
                max_items=SUMMARY_BATCH_LIMIT,
            )
            workset_sha256 = clamp_sha256(payload["workset_sha256"], field_name="workset_sha256")
            generator = payload["generator"]
            if type(generator) is not dict or set(generator) != {"model_id", "prompt_sha256"}:
                raise ValueError("generator must contain exactly model_id and prompt_sha256")
            if type(generator["model_id"]) is not str:
                raise ValueError("generator.model_id must be a string")
            model_id = clamp_text(generator["model_id"], field_name="generator.model_id", max_len=200)
            prompt_sha256 = clamp_sha256(
                generator["prompt_sha256"],
                field_name="generator.prompt_sha256",
            )
        ...
        with connect_db(self.server_state["db_path"]) as conn:
            ...
            expire_stale_claims(conn)
            head_row = fetch_summary_head_row(conn, thread)
            current_parent_revision_id = int(head_row["id"]) if head_row is not None else 0
            if current_parent_revision_id != expected_parent_revision_id:
                conn.commit()
                self._json_error(HTTPStatus.CONFLICT, "summary parent revision changed; request a fresh workset")
                return
            try:
                workset = build_summary_workset(
                    conn,
                    thread=thread,
                    coverage_through_message_id=coverage_through_message_id,
                )
            except ValueError as exc:
                conn.commit()
                self._json_error(HTTPStatus.BAD_REQUEST, str(exc))
                return
            if workset["task_event_head_id"] != expected_task_event_head_id:
                conn.commit()
                self._json_error(HTTPStatus.CONFLICT, "Taskboard event head changed; request a fresh workset")
                return
            if workset["task_snapshot_sha256"] != expected_task_snapshot_sha256:
                conn.commit()
                self._json_error(HTTPStatus.CONFLICT, "Taskboard snapshot changed; request a fresh workset")
                return
            if workset["workset_sha256"] != workset_sha256:
                conn.commit()
                self._json_error(HTTPStatus.CONFLICT, "summary workset changed; request a fresh workset")
                return
            if workset["batch_message_ids"] != batch_message_ids:
                conn.commit()
                self._json_error(HTTPStatus.BAD_REQUEST, "batch_message_ids do not match the covered thread batch")
                return

            allowed_source_ids = set(workset["parent_source_message_ids"])
            allowed_source_ids.update(workset["batch_message_ids"])
            try:
                draft = validate_summary_draft(
                    payload["draft"],
                    allowed_source_message_ids=allowed_source_ids,
                )
                existing_task_titles = [
                    str(row[0])
                    for row in conn.execute(
                        "SELECT title FROM tasks WHERE thread = ?",
                        (thread,),
                    ).fetchall()
                ]
                validate_task_proposals_are_new(
            ...
            body = render_summary_markdown(draft)
            now = utc_now()
            output_sha256 = hashlib.sha256(body.encode("utf-8")).hexdigest()
            cursor = conn.execute(
                """
                INSERT INTO summary_revisions (
                    thread, created_ts, created_by, kind, parent_revision_id,
                    coverage_through_message_id, coverage_known, task_event_head_id,
                    task_snapshot_sha256, input_sha256, model_id, prompt_sha256,
                    content_json, body, output_sha256
                ) VALUES (?, ?, ?, 'steward', ?, ?, 1, ?, ?, ?, ?, ?, ?, ?, ?)
                """,
                (
                    thread,
            ...
            conn.executemany(
                """
                INSERT INTO summary_message_sources (revision_id, claim_path, message_id)
                VALUES (?, ?, ?)
                """,
                [
                    (revision_id, claim_path, message_id)
                    for claim_path, message_id in summary_draft_source_rows(draft)
                ],
            )
            if expected_parent_revision_id == 0:
                conn.execute(
                    "INSERT INTO summary_heads (thread, revision_id) VALUES (?, ?)",
                    (thread, revision_id),
                )
            else:
                updated = conn.execute(
                    "UPDATE summary_heads SET revision_id = ? WHERE thread = ? AND revision_id = ?",
                    (revision_id, thread, expected_parent_revision_id),
                )
                if updated.rowcount != 1:
                    conn.rollback()
                    self._json_error(HTTPStatus.CONFLICT, "summary head changed during publication")
                    return
            conn.execute(
                """
                INSERT INTO summaries (thread, updated_ts, updated_by, body)
                VALUES (?, ?, ?, ?)
            ...
            revision_row = conn.execute("SELECT * FROM summary_revisions WHERE id = ?", (revision_id,)).fetchone()
            conn.commit()

        self._json_response(
            {"ok": True, "summary": summary_revision_row_to_dict(revision_row)},
            status=HTTPStatus.CREATED,
        )
    ...
    def _handle_update_summary(self, auth: AuthContext) -> None:
        ...
        with connect_db(self.server_state["db_path"]) as conn:
            begin_immediate(conn)
            head_row = fetch_summary_head_row(conn, thread)
            parent_revision_id = int(head_row["id"]) if head_row is not None else 0
            coverage_through_message_id = 0
            coverage_known = 0
            taskboard = build_summary_taskboard_snapshot(conn, thread)
            input_sha256 = sha256_json({"kind": "manual", "body": body})
            output_sha256 = hashlib.sha256(body.encode("utf-8")).hexdigest()
            cursor = conn.execute(
                """
                INSERT INTO summary_revisions (
            ...
            conn.execute(
                """
                INSERT INTO summary_heads (thread, revision_id) VALUES (?, ?)
                ON CONFLICT(thread) DO UPDATE SET revision_id=excluded.revision_id
                """,
                (thread, revision_id),
            )
            conn.execute(
