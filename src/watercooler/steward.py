...
from .common import WatercoolerError, build_direct_http_opener, load_config, request_json
...
def call_chat_completions(
    *,
    ...
    try:
        with build_direct_http_opener().open(request, timeout=timeout) as response:
            if response.geturl() != request.full_url:
                raise StewardError("model endpoint changed without authorization")
            response_body = response.read(MAX_MODEL_RESPONSE_BYTES + 1)
            if len(response_body) > MAX_MODEL_RESPONSE_BYTES:
                raise StewardError("model response exceeds its byte limit")
            response_text = response_body.decode("utf-8")
    except urllib.error.HTTPError as exc:
        detail = exc.read(MAX_MODEL_ERROR_BYTES + 1)[:MAX_MODEL_ERROR_BYTES].decode(
            "utf-8",
            errors="replace",
        )
