...
**Status:** review candidate; no Phase 2b outcome collection is authorized.
...
- A failed or short run is never replaced with a newly chosen seed. Crash
  recovery may resume the same hash-bound journal only.
...
A run is positive only when all four within-run deltas are strictly greater
than zero. At least 12 of 16 runs must be positive.
...
| transitions per evaluation run | `>= 16` |

The `512` total is the v1 preregistered average minimum of 32 transitions per
run scaled to 16 runs. The per-run minimum and all effect/consistency thresholds
...
1. Is 16 runs with the preserved `>= 0.75` rule (`>= 12/16`) adequate for the
   stated consistency replication, or should the run count be larger before
   any outcome exists?
2. Is scaling the v1 total-support minimum to `512` while retaining the v1
   per-run minimum of `16` the correct failure policy for terminal runs?
3. Should a scoped Phase 2b GO remain `GO_LS20_CONSISTENCY_REPLICATED`, as
   proposed, rather than directly authorizing the later learned-observer step?
