...
1. **Directions:** ≥2 DC-removed unit directions per tooth: (a) a 5g.3 MVB disposition direction, (b) `results/oxytocin_extraction/gemma4_12b_oxytocin_v1.pt` (Gemini's G0 artifact — DC-subtracted, extracted at these exact teeth; this run doubles as its first envelope-validated use, per its own `envelope_status` marker). Positive-valence directions only (per #671 ordering; Cairn's valence-asymmetric class untouched).
