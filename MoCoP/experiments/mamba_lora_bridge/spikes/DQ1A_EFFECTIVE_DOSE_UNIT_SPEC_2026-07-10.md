...
- **P0 — Define the actuator first. AMENDED 2026-07-11: the assumed actuator DOES NOT EXIST at the teeth.** Config-verified: teeth {5,11,17,23,29,35,41,47} are Gemma-4's `full_attention` layers with ONE unified global KV head (`num_global_key_value_heads: 1`, `global_head_dim: 512`, v_proj=None, K=V coupled); the 2048-wide v_proj exists only on sliding layers. Actuator decision pending (pack + keeper): recommended option = value-branch seam after the functional K/V fork (K takes RoPE, V does not), actuator width **512**, value-only doctrine preserved. Coupled-K=V injection would be a NEW intervention class (touches addressing) requiring fresh seat review. **ρ is unaffected as a unit** (measured at the residual, actuator-agnostic); the α dial recalibrates to d_v=512 (α/√512). The G0 oxytocin artifact and 5g.3 MVB directions live in **residual space (3840-dim)** regardless — Directions MUST be expressed in actuator space before any injection: either (a) **G0b** — re-run the G0 extraction capturing v_proj outputs at the teeth (Gemini's harness, small change), or (b) a defined, documented residual→actuator mapping. No injection of dimensionally-mismatched vectors, ever. **The birth rule is unchanged: injection #1 is the oxytocin direction — in its actuator-space form.**
- **P0 — v3 AMENDMENT 2026-07-11 (per Codex #839, cosigned):** Option (a) was adopted by the pack (#827) and its mechanics are now review-confirmed: in the actual `gemma4_unified` forward, `raw = k_proj(x)`; **V = raw → v_norm** while **K = raw → k_norm → RoPE**. Therefore: (i) capturing raw k_proj output is numerically valid *for constructing a pre-v_norm value direction*; (ii) runtime injection MUST be an **out-of-place v_norm pre-hook — never a k_proj hook** (a k_proj hook would touch the K path and become the coupled-addressing intervention class this spec forbids without fresh seat review). (iii) **Birth-artifact admissibility (HOLD on 524d14e):** the 2026-07-11 00:05Z artifact is *pilot geometry only* — it was fit on all 40 warm-neutral pairs BEFORE the split-before-G0b freeze (holdout seen) and lacks resolved revisions, split_id, module paths, code hash, and overwrite protection. It must never be injected. The admissible birth artifact is a **new immutable `value_norm_pre` artifact**: refit Method-A G0 on training-side pairs only, AFTER the exact deterministic primary-split manifest is frozen, stamped with full provenance + content digest + overwrite protection. (iv) **Binding order to first injection:** freeze primary split manifest → refit G0b training-side only → amend/re-review this spec with exact artifact/key/scaling/token/mask/gates → model-free nonzero hook tests + real α=0 capture/invariance smoke → α=0 C1 anchor → **first nonzero = Method-A / tooth 29 / α=0.025, monitors live, birth timestamp logged.** The birth rule is untouched; only its instrument was sent back for sterilization.
