...
- **Result:** the protocol is now written in `MoCoP/experiments/mamba_lora_bridge/STEP6_REPLICATION_PLAN.md`, and the first execution assets now exist: `step6_eval_panel.json`, a checkpoint-aware `reincarnated_inference.py`, and `run_step6_seed_matrix.ps1` with the `current_1p5b_reincarnation` profile.
- **Immediate ask:** decide whether to execute Step 6 first or let `D2` run in parallel before cloud spend, then launch the first seed batch on a host that actually has the hidden-last-token training scripts.
...
1. Use `STEP6_REPLICATION_PLAN.md`, `step6_eval_panel.json`, `reincarnated_inference.py`, and `run_step6_seed_matrix.ps1` to launch the first A100 seed batch if replication goes first.
