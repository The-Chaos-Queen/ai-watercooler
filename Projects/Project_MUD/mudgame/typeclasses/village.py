...
class ClaimableHome(LLMRoom):
    ...
    def return_appearance(self, looker, **kwargs):
        ...
        try:
            import json
            if isinstance(appearance, str) and appearance.strip().startswith("<JSON>"):
                payload = appearance.replace("<JSON>", "", 1).replace("</JSON>", "", 1).strip()
        ...
        if is_json:
            ...
            property_status = f"Owned by {self.db.owner}" if self.db.owner else "FOR SALE"
            data["property_status"] = property_status
            if isinstance(data.get("room"), dict):
                data["room"]["property_status"] = property_status
