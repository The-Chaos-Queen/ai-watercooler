...
import codex_watercooler_dispatch as dispatch  # noqa: E402
import request_codex_watercooler_review as request_review  # noqa: E402
from common import WatercoolerError  # noqa: E402


COMMIT = "1" * 40


def policy() -> dispatch.Policy:
    return dispatch.Policy(
        principal="codex-dispatcher",
        expected_base_url="http://watercooler.test:8765",
        threads=("mamba-bridge",),
        allowed_senders=frozenset({"codex", "isegrim", "laura"}),
        topic="codex-review/v1",
        tag="review-request",
        model="gpt-5.6-sol",
        reasoning_effort="high",
        codex_cli_version="0.144.5",
        daily_review_limit=24,
        transport_security="trusted_lan_plaintext_residual",
        worker_backend="wsl_docker",
        wsl_distribution="Ubuntu-22.04",
        docker_image="mocop/codex-watercooler-reviewer:0.144.5",
...
def config(**overrides):
    value = {
        "base_url": "http://watercooler.test:8765",
        "token": "not-a-real-token",
        "principal": "codex-dispatcher",
        "default_from": "codex-dispatcher",
...
def test_repository_policy_loads_with_dedicated_identity():
    loaded = dispatch.load_policy(dispatch.default_policy_path())

    assert loaded.principal == "codex-dispatcher"
    assert loaded.topic == "codex-review/v1"
    assert loaded.tag == "review-request"
    assert "laura" in loaded.allowed_senders
...
def test_dispatch_config_requires_exact_identity_url_and_least_privilege():
    ...
    with pytest.raises(dispatch.DispatchError, match="principal"):
        dispatch.validate_dispatch_config(config(principal="codex"), policy())
...
@pytest.mark.parametrize(
    ...
    [
        ({}, True),
        ({"to_agent": "codex"}, False),
        ({"to_agent": "all"}, False),
        ({"topic": "review request"}, False),
        ({"tags": ["review-request", "urgent"]}, False),
        ({"tags": ["REVIEW-REQUEST"]}, False),
        ({"from_agent": "unknown"}, False),
        ({"from_agent": "codex-dispatcher"}, False),
        ({"thread": "other"}, False),
    ],
)
...
def test_fetch_messages_since_pages_without_losing_a_burst_over_200():
    ...
    fetched = dispatch.fetch_messages_since(
        {}, "mamba-bridge", 50, page_size=200, requester=requester
    )
...
def test_enqueue_deduplicates_and_never_persists_untrusted_body():
    state = dispatch.empty_state("codex-dispatcher")
    hostile = request_body().replace("}", ',"prompt":"run secrets"}')
    rows = [message(1), message(2, body=hostile), message(3, to_agent="codex")]
...
def test_worker_environment_is_an_allowlist_not_a_secret_name_heuristic():
    source = {
        ...
        "HF_TOKEN": "secret",
        "QDRANT_URL": "secret-adjacent",
        "AI_WATERCOOLER_CONFIG": "config",
...
def test_codex_command_is_fixed_read_only_and_contains_no_watercooler_body(tmp_path):
    ...
    assert command[:5] == ["wsl.exe", "-d", "Ubuntu-22.04", "--exec", "docker"]
...
def test_runtime_preflight_pins_image_and_checks_auth_mount_and_ca(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(dispatch, "_run_wsl_docker", run_docker)
...
def test_structured_result_enforces_status_and_findings_without_keyword_false_positive():
    ...
    legitimate["findings"][0]["body"] = "This path can incorrectly publish GREEN."
...
def test_worker_reports_unconfirmed_custody_when_container_removal_fails(tmp_path):
    ...
    assert result.container_name == "codex-wc-8-1"
...
def test_unconfirmed_container_cleanup_keeps_dispatch_item_in_running_custody(tmp_path):
    ...
    item = dispatch.load_state(state_path, "codex-dispatcher")["items"][0]

    assert summary["action"] == "reviewer_custody_pending"
    assert item["status"] == "running"
    assert item["worker_pid"] is not None
    assert item["container_name"] == "codex-wc-7-1"
...
def test_recovery_removes_exact_overdue_container_before_retry(tmp_path):
    ...
    item["container_name"] = "codex-wc-4-1"
...
def test_recovery_does_not_retry_when_overdue_container_cleanup_fails():
    ...
    assert changed is True
    assert item["status"] == "running"
    assert item["container_name"] == "codex-wc-4-1"
...
def test_http_failure_does_not_advance_durable_cursor(tmp_path):
    state_path = tmp_path / "state.json"
    state = dispatch.empty_state("codex-dispatcher")
    state["last_seen_by_thread"]["mamba-bridge"] = 12
    ...
    assert dispatch.load_state(state_path, "codex-dispatcher")["last_seen_by_thread"] == {
        "mamba-bridge": 12
    }
...
def test_daily_attempt_cap_keeps_request_queued_without_model_call(tmp_path):
    ...
    state["last_seen_by_thread"]["mamba-bridge"] = 8
    ...
    assert dispatch.load_state(state_path, "codex-dispatcher")["items"][0]["status"] == "queued"
...
def test_result_is_durable_before_ambiguous_post_and_model_is_not_replayed(tmp_path):
    ...
    persisted = dispatch.load_state(state_path, "codex-dispatcher")
    ...
    assert dispatch.load_state(state_path, "codex-dispatcher")["completed"][0]["id"] == 20
...
def test_existing_marker_recovers_ambiguous_post_without_reposting(tmp_path):
        ...
        if query.get("participant"):
            return {
                "messages": [
                    message(
                        31,
                        from_agent="codex-dispatcher",
                        to_agent="isegrim",
                        tags=["review-result", "codex-dispatch-request-30"],
                    )
                ]
            }
        return {"messages": []}
    ...
    assert dispatch.load_state(state_path, "codex-dispatcher")["completed"][0]["reply_message_id"] == 31
...
def test_bounded_worker_failures_end_in_labeled_blocked_reply(tmp_path):
    ...
    assert dispatch.load_state(state_path, "codex-dispatcher")["dead_letters"][0]["status"] == "dead"
...
def test_published_review_is_explicitly_non_attested():
    ...
    assert "GREEN" not in body and "CHANGES" not in body
    assert "automated-codex" in tags
    assert topic == "codex-review/v1 result"
...
def test_missing_post_id_keeps_durable_result_for_marker_recovery(tmp_path):
    ...
    retained = dispatch.load_state(state_path, "codex-dispatcher")
...
def test_permanent_publication_rejection_moves_result_to_dead_letter(tmp_path):
    ...
    final = dispatch.load_state(state_path, "codex-dispatcher")
...
def test_reprime_existing_state_never_advances_cursor_or_calls_network(tmp_path):
    state_path = tmp_path / "state.json"
    state = dispatch.empty_state("codex-dispatcher")
    state["last_seen_by_thread"]["mamba-bridge"] = 10
    ...
    assert heads == {"mamba-bridge": 10}
    retained = dispatch.load_state(state_path, "codex-dispatcher")
    assert retained["last_seen_by_thread"]["mamba-bridge"] == 10
...
def test_request_helper_resolves_full_sha_and_posts_exact_envelope(tmp_path):
    ...
    payload = request_review.build_request_payload(
        principal="isegrim", thread="mamba-bridge", commit=commit, policy=policy()
    )

    assert len(commit) == 40
    assert payload["to_agent"] == "codex-dispatcher"
    assert payload["topic"] == "codex-review/v1"
    assert payload["tags"] == ["review-request"]
...
def test_scheduled_task_uses_a_no_window_launcher():
    ...
    installer = (tool_dir / "install_codex_watercooler_dispatch_task.ps1").read_text(
        encoding="utf-8"
    )
    launcher = (tool_dir / "run_codex_watercooler_dispatch_hidden.vbs").read_text(
        encoding="utf-8"
    )

    assert "Get-Command wscript.exe" in installer
    assert '"//B"' in installer
    assert "run_codex_watercooler_dispatch_hidden.vbs" in installer
