...
def handle_reopen(args: argparse.Namespace, config: Dict[str, Any]) -> int:
    return handle_state_change(args, config, path="/v1/tasks/reopen", extra_payload={})
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Watercooler Taskboard CLI")
    ...
    reopen_parser = subparsers.add_parser("reopen", help="Move a done task back to queued with its history intact.")
    add_agent_arg(reopen_parser)
    reopen_parser.add_argument("--task-id", type=int, required=True)
    reopen_parser.add_argument("--note", type=str, default="")
    reopen_parser.add_argument("--json", action="store_true")
    reopen_parser.set_defaults(handler=handle_reopen)
