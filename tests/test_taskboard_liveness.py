...
def watercooler(tmp_path):
    ...
    ensure_db(db_path)
    tokens = {"agent-a": "token-agent-a", "readonly": "token-readonly"}
    now = utc_now()
...
def test_liveness_endpoint_reports_blocked_age_and_signals(watercooler):
    ...
    done = create_task(base_url, tokens["agent-a"], title="replacement slice", description="done replacement")
    fresh = create_task(base_url, tokens["agent-a"], title="fresh ethics gate", description="waiting for QC")
    ...
    request_json(
        ...
        "POST",
        "/v1/tasks/complete",
        {"task_id": done["id"], "agent": "agent-a", "note": "done"},
    )
    request_json(
        base_url,
        tokens["agent-a"],
        "POST",
        "/v1/tasks/block",
        {"task_id": stale["id"], "agent": "agent-a", "blocked_reason": f"superseded by #{done['id']}"},
    )
    request_json(
        base_url,
        tokens["agent-a"],
        "POST",
        "/v1/tasks/block",
        {"task_id": fresh["id"], "agent": "agent-a", "blocked_reason": "waiting for QC gate"},
    )
    ...
    report = request_json(
        ...
        tokens["agent-a"],
        "GET",
        "/v1/tasks/liveness?" + urllib.parse.urlencode({"project": "demo", "now": "2026-05-29T00:00:00Z"}),
    )
...
def test_liveness_endpoint_requires_tasks_read_auth(watercooler):
    ...
    code, body = request_error(base_url, "not-a-real-token", "GET", "/v1/tasks/liveness?project=demo")
