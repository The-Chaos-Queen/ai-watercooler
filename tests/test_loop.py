...
def test_default_state_root_uses_public_watercooler_namespace(tmp_path, monkeypatch):
    if loop.os.name == "nt":
        monkeypatch.setenv("LOCALAPPDATA", str(tmp_path))
    else:
        monkeypatch.setenv("XDG_STATE_HOME", str(tmp_path))

    assert loop.LoopPaths.default().root == tmp_path / "Watercooler" / "loops"
